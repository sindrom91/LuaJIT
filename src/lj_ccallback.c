...
static void callback_mcode_init(global_State *g, uint32_t *page)
{
  ...
  void *target = (void *)lj_vm_ffi_callback;
  MSize slot;
  *p++ = A64I(A64I_LDRLx | A64F_D(RID_X11) | A64F_S19(4));
  *p++ = A64I(A64I_LDRLx | A64F_D(RID_X10) | A64F_S19(5));
  *p++ = A64I(A64I_BR | A64F_N(RID_X11));
  *p++ = A64I(A64I_NOP);
  ((void **)p)[0] = target;
  ((void **)p)[1] = g;
  p += 4;
  for (slot = 0; slot < CALLBACK_MAX_SLOT; slot++) {
    *p++ = A64I(A64I_MOVZw | A64F_D(RID_X9) | A64F_U16(slot));
    *p = A64I(A64I_B | A64F_S26((page-p) & 0x03ffffffu));
    p++;
  }
  lua_assert(p - page <= CALLBACK_MCODE_SIZE);
}
