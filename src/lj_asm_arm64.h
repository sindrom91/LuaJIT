...
static void asm_exitstub_setup(ASMState *as, ExitNo nexits)
{
  ...
  for (i = nexits-1; (int32_t)i >= 0; i--)
    *--mxp = A64I(A64I_BL|((-3-i)&0x03ffffffu));
  *--mxp = A64I(A64I_MOVZw|A64F_U16(as->T->traceno));
  mxp--;
  *mxp = A64I(A64I_BL|(((MCode *)(void *)lj_vm_exit_handler-mxp)&0x03ffffffu));
  *--mxp = A64I(A64I_STRx|A64F_D(RID_LR)|A64F_N(RID_SP));
  as->mctop = mxp;
}
...
#if LJ_BE
/* Byteswap machine code on big-endian. */
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
  ...
  for (; p < pe; p++) {
	...
	((ins ^ ((px-p)<<5)) & 0x00ffffe0u) == 0) {
      /* Patch bcc exitstub. */
      *p = A64I((ins & 0xff00001fu) | (((target-p)<<5) & 0x00ffffe0u));
      cend = p+1;
      if (!cstart) cstart = p;
    } else if ((ins & 0xfc000000u) == 0x14000000u &&
	       ((ins ^ (px-p)) & 0x03ffffffu) == 0) {
      /* Patch b exitstub. */
      *p = A64I((ins & 0xfc000000u) | ((target-p) & 0x03ffffffu));
      cend = p+1;
      if (!cstart) cstart = p;
    } else if ((ins & 0x7e000000u) == 0x34000000u &&
	       ((ins ^ ((px-p)<<5)) & 0x00ffffe0u) == 0) {
      /* Patch cbz/cbnz exitstub. */
      *p = A64I((ins & 0xff00001f) | (((target-p)<<5) & 0x00ffffe0u));
      cend = p+1;
      if (!cstart) cstart = p;
    } else if ((ins & 0x7e000000u) == 0x36000000u &&
	       ((ins ^ ((px-p)<<5)) & 0x0007ffe0u) == 0) {
      /* Patch tbz/tbnz exitstub. */
      *p = A64I((ins & 0xfff8001fu) | (((target-p)<<5) & 0x0007ffe0u));
      cend = p+1;
      if (!cstart) cstart = p;
    }
  }
