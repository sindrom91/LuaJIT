...
#define asm_popcnt64(n)	__builtin_popcountll(n)
#define asm_ffs64(n)	__builtin_ffsll(n)
...
static void asm_intcomp(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(rref) && get_k64val(IR(rref)) == 0) {
	...
	if ((cc == CC_EQ || cc == CC_NE) && asm_popcnt64(k) == 1) {
	  asm_guardtnb(as, cc == CC_EQ ? A64I_TBZ : A64I_TBNZ,
		       ra_alloc1(as, blref, RSET_GPR), asm_ffs64(k) - 1);
	  return;
	}
