...
#define MCLINK_SIZE		(LJ_64 ? 16 : 8)

/* Setup spare long-range jump slots per mcarea. */
static void asm_sparejump_setup(ASMState *as)
{
  MCode *mxp = as->mcbot;
  if (((uintptr_t)mxp & (LJ_PAGESIZE-1)) == MCLINK_SIZE) {
    lua_assert(MIPSI_NOP == 0);
    /* CHECK: Why +2? MCLink should already be excluded from as->mcbot. */
...
#else
	  emit_spstore(as, ir, r,
	    ofs + ((LJ_BE && !irt_isfp(ir->t) && !irt_is64(ir->t)) ? 4 : 0));
	  ofs += 8;
#endif
	}
      }
...
#elif LJ_64
#define asm_abs(as, ir) \
  emit_tsml(as, MIPSI_DEXTM, ra_dest(as, ir, RSET_GPR), \
	    ra_alloc1(as, ir->op1, RSET_GPR), 30, 0);
#endif
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
  ...
  for (p++; p < pe; p++) {
    if (*p == exitload) {  /* Look for load of exit number. */
      ...
      if (((p[-1] ^ (px-p)) & 0xffffu) == 0 && is_branch(p[-1])) {
	...
	patchbranch:
	  ...
	  *p = MIPSI_NOP;  /* Replace the load of the exit number. */
	  ...
	  int i;
	  /* Skip MCLink, which is at the beginning of mcarea. */
	  for (i = MCLINK_SIZE/4; i < MCLINK_SIZE/4+MIPS_SPAREJUMP*2; i += 2) {
	    if (mcarea[i] == tjump) {
	      delta = mcarea+i - p;
	      goto patchbranch;
