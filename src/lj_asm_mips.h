...
#else
	  emit_spstore(as, ir, r, ofs + ((LJ_BE && irt_isint(ir->t)) ? 4 : 0));
	  ofs += 8;
#endif
	}
      }
...
static void asm_retf(ASMState *as, IRIns *ir)
{
  ...
  asm_guard(as, MIPSI_BNE, RID_TMP,
	    ra_allock(as, intptr(pc), rset_exclude(RSET_GPR, base)));
...
static void asm_tobit(ASMState *as, IRIns *ir)
{
  ...
  emit_tg(as, MIPSI_MFC1, dest, tmp); //TODO
...
static void asm_conv(ASMState *as, IRIns *ir)
{
  ...
  if (irt_isfp(ir->t)) {
    ...
    if (stfp) {  /* FP to FP conversion. */
      ...
      /* if (x>2^31-1) y = (x ^ 0x8000000) + 2147483648.0 */
	...
	emit_lsptr(as, MIPSI_LDC1, (tmp & 31),
      ...
      emit_dst(as, MIPSI_XOR, left, RID_TMP, left);
      emit_dst(as, MIPSI_AND, RID_TMP, RID_TMP, left);
      emit_ti(as, MIPSI_LUI, RID_TMP, 0x8000);
#if LJ_64
    } else if(st == IRT_U64) {  /* U64 to FP conversion. */
      /* if (x>2^63-1) y = (x ^ 2^63) + 2^63 */
      Reg left = ra_alloc1(as, lref, RSET_GPR);
      Reg tmp = ra_scratch(as, rset_exclude(RSET_FPR, dest));
      MCLabel l_end = emit_label(as);
      /* Add 2^64 to compensate. */
      emit_fgh(as, MIPSI_ADD_D, dest, dest, tmp);
      emit_lsptr(as, MIPSI_LDC1, (tmp & 31), (void *)&as->J->k64[LJ_K64_2P64],
		 rset_exclude(RSET_GPR, left));
      emit_fg(as, irt_isfloat(ir->t) ? MIPSI_CVT_S_L : MIPSI_CVT_D_L,
      	      dest, dest);
      emit_branch(as, MIPSI_BEQ, RID_TMP, RID_ZERO, l_end);
      emit_tg(as, MIPSI_DMTC1, left, dest);
      /* Check if >= 2^63. */
      emit_dst(as, MIPSI_SLT, RID_TMP, left, RID_ZERO);
#endif
	...
	MCLabel l_end;
	if (st == IRT_NUM) {
	  emit_tg(as, MIPSI_DMFC1, dest, left);
	  emit_fg(as, MIPSI_TRUNC_L_D, left, left);
	  l_end = emit_label(as);
	  /* For inputs >= 2^63 add -2^64. */
	  emit_fgh(as, MIPSI_ADD_D, left, left, tmp);
	  emit_lsptr(as, MIPSI_LDC1, (tmp & 31),
	  ...
	  l_end = emit_label(as);
	  /* For inputs >= 2^31 add -2^31, convert again and add 2^31. */
	  emit_tsml(as, MIPSI_DEXT, dest, dest, 31, 0);
	  emit_dst(as, MIPSI_XOR, dest, RID_TMP, dest);
	  emit_tg(as, MIPSI_MFC1, dest, left);
          emit_ti(as, MIPSI_LUI, RID_TMP, 0x8000);
	  emit_fg(as, MIPSI_TRUNC_L_S, left, left);
	  emit_fgh(as, MIPSI_SUB_S, left, left, tmp);
	  emit_lsptr(as, MIPSI_LWC1, (tmp & 31),
		     (void *)&as->J->k32[LJ_K32_2P31],
		     rset_exclude(RSET_GPR, dest));
          emit_branch(as, MIPSI_BEQ, RID_TMP, RID_ZERO, l_end);
          emit_dst(as, MIPSI_SLTU, RID_TMP, RID_TMP, dest);
	  emit_tg(as, MIPSI_MFC1, dest, tmp);
          emit_ti(as, MIPSI_LUI, RID_TMP, 0x8000);
...
static void asm_tvptr(ASMState *as, Reg dest, IRRef ref)
{
  ...
  if (irt_isnum(ir->t)) {
    if (irref_isk(ref))  /* Use the number constant itself as a TValue. */
      ra_allockreg(as, intptr(ir_knum(ir)), dest);
    ...
    if (irref_isk(ref)) {
      ...
      emit_tsi(as, MIPSI_SD, src, dest, 0);
    } else {
      Reg type;
      if (irt_ispri(ir->t)) {
	...
	emit_tsi(as, MIPSI_SD, RID_TMP, dest, 0);
...
static void asm_hrefk(ASMState *as, IRIns *ir)
{
  ...
  int32_t lo, hi;
#else
  Reg kkey, key = ra_scratch(as, allow);
#endif
...
#else
  if (!irt_ispri(irkey->t)) {
    kkey = ra_allock(as, irt_isnum(irkey->t) ? ir_knum(irkey)->u64 :
			    ((uint64_t)irt_toitype(irkey->t) << 47) |
			    (uint64_t)ir_kgc(irkey), allow);
    lua_assert(irt_isnum(irkey->t) || irt_isgcv(irkey->t));
  } else {
    kkey = ra_allock(as, ~((int64_t)~irt_toitype(irkey->t) << 47), allow);
    lua_assert(!irt_isnil(irkey->t));
  }
  asm_guard(as, MIPSI_BNE, key, kkey);
...
static void asm_ahuvload(ASMState *as, IRIns *ir)
{
  ...
  if (ra_used(ir)) {
#if LJ_32
    lua_assert((LJ_SOFTFP ? 0 : irt_isnum(t)) || irt_isint(t) || irt_isaddr(t));
    dest = ra_dest(as, ir, (!LJ_SOFTFP && irt_isnum(t)) ? RSET_FPR : allow);
    rset_clear(allow, dest);
#else
    int hfnum = (irt_isnum(t));
    dest = ra_dest(as, ir, hfnum ? RSET_FPR : allow);
    lua_assert(irt_isnum(t) || irt_isint(t) || irt_isaddr(t));
    val = hfnum ? ra_scratch(as, rset_clear(allow, dest)) : dest;
    ...
    if (irt_isaddr(t))
      ...
      emit_tg(as, MIPSI_DMTC1, val, dest);
#endif
  }
...
#else
  emit_dta(as, MIPSI_DSRA32, type, val, 15);
  emit_tsi(as, MIPSI_LD, val, idx, ofs);
#endif
}
...
dotypecheck:
  ...
  if ((ir->op2 & IRSLOAD_TYPECHECK)) {
    Reg temp = RID_TMP;
    if (ra_noreg(dest) || rset_test(RSET_FPR, dest)) {
      Reg r = ra_scratch(as, allow);
      if (rset_test(RSET_FPR, dest))
	emit_tg(as, MIPSI_DMTC1, r, dest);
      dest = r;
      rset_clear(allow, dest);
    }
    if (irt_isnum(t)) {
      asm_guard(as, MIPSI_BEQ, temp, RID_ZERO);
      emit_tsi(as, MIPSI_SLTIU, temp, temp, (int32_t)LJ_TISNUM);
      emit_dta(as, MIPSI_DSRA32, temp, dest, 15);
    } else if (irt_isint(t)) {
      Reg ktype = ra_allock(as, (int32_t)irt_toitype(t), allow);
      asm_guard(as, MIPSI_BNE, temp, ktype);
      emit_dta(as, MIPSI_DSRA32, temp, dest, 15);
    } else if (irt_isnil(t)) {
      asm_guard(as, MIPSI_BNE, dest, RID_ZERO);
      emit_tsi(as, MIPSI_DADDIU, dest, dest, 1);
    } else if (irt_ispri(t)) {
      asm_guard(as, MIPSI_BNE, dest,
		ra_allock(as, ~((int64_t)~irt_toitype(t) << 47) , allow));
    } else {
      asm_guard(as, MIPSI_BNE, temp, RID_ZERO);
      emit_tsi(as, MIPSI_DADDIU, temp, temp, -irt_toitype(t));
      emit_dta(as, MIPSI_DSRA32, temp, dest, 15);
    }
    emit_tsi(as, MIPSI_LD, dest, base, ofs);
    return;
  }
  if (ra_hasreg(dest)) {
    if (irt_isnum(t))
...
static void asm_cnew(ASMState *as, IRIns *ir)
{
  ...
  if (ir->o == IR_CNEWI) {
    ...
    emit_tsi(as, MIPSI_SD, r, RID_RET, sizeof(GCcdata));
#endif
...
static void asm_arithov(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op2)) {
    ...
    if (checki16(k)) {  /* (dest < left) == (k >= 0 ? 1 : 0) */
      ...
      emit_tsi(as, (LJ_64 && irt_is64(ir->t)) ? MIPSI_DADDIU : MIPSI_ADDIU,
      	       dest, left, k);
  ...
  if (ir->o == IR_ADDOV)
    emit_dst(as, (LJ_64 && irt_is64(ir->t)) ? MIPSI_DADDU : MIPSI_ADDU, dest,
    	     left, right);
  else
    emit_dst(as, (LJ_64 && irt_is64(ir->t)) ? MIPSI_DSUBU : MIPSI_SUBU, dest,
    	     left, right);
...
static void asm_bitshift(ASMState *as, IRIns *ir, MIPSIns mi, MIPSIns mik)
{
  ...
  if (irref_isk(ir->op2)) {  /* Constant shifts. */
    uint32_t shift = (uint32_t)(get_kval(IR(ir->op2)));
    if (LJ_64 && irt_is64(ir->t)){
      if (shift >= 32)
	emit_dta(as, mik|MIPSI_D32, dest,
		 ra_hintalloc(as, ir->op1, dest, RSET_GPR), shift - 32);
      else
	emit_dta(as, mik|MIPSI_D, dest,
		 ra_hintalloc(as, ir->op1, dest, RSET_GPR), shift);
    } else {
    ...
    Reg right, left = ra_alloc2(as, ir, RSET_GPR);
    right = (left >> 8); left &= 255;
    emit_dst(as, (LJ_64 && irt_is64(ir->t)) ? (mi|MIPSI_DV) : mi, dest, right,
    	     left);  /* Shift amount is in rs. */
  }
}
...
#if LJ_64
#define asm_bror(as, ir)	asm_bitshift(as, ir, MIPSI_ROTRV, MIPSI_ROTR)
#else
...
static void asm_stack_restore(ASMState *as, SnapShot *snap)
{
  ...
  for (n = 0; n < nent; n++) {
	...
	if (irt_is64(ir->t)) {
	  ...
	  type = ra_allock(as, ~((int64_t)~irt_toitype(ir->t) << 47), allow);
	  emit_tsi(as, MIPSI_SD, type, RID_BASE, ofs);
	}
      } else {
	TValue k;
	lj_ir_kvalue(as->J->L, &k, ir);
